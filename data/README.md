...
| `eval_cvs.csv` | 17 | 15 labelled benchmark CVs used in the reported benchmark run, plus 2 adversarial demo profiles (`is_eval = false`). Columns: `cv_id, name, track, level, is_eval, relevant_job_ids, raw_text`. `relevant_job_ids` is the ground-truth relevance set used to compute Precision@5. |
